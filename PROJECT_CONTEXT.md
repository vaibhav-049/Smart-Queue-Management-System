...
- `/`: home page.
- `/login`: login page.
- `/register`: OTP-backed registration flow.
...
The root `package.json` currently only declares `axios` as a dependency and does not define scripts for running both apps together.

## Default Seed Data

`backend/src/config/db.js` seeds:
- Services: hospital, bank, college, government, salon.
- Queue containers for each service.
- Admin user: `vikram@example.com` with password `admin123`.

Note: `backend/src/controllers/serviceController.js` has a fallback service seed list that differs from `db.js`; it includes restaurant and omits college/salon. Keeping one canonical service seed list would avoid inconsistent behavior.
...
- `frontend/src/context/AuthContext.jsx`: auth/session/socket state.
